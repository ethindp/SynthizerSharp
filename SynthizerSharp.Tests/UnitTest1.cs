...
var library = FFIActivator.ActivateFFIInterface();
        ...
        Assert.That(library.syz_handleIncRef(h), Is.EqualTo(0));
        Assert.That(library.syz_handleDecRef(h), Is.EqualTo(0));
        Assert.That(library.syz_handleDecRef(h), Is.EqualTo(0));
...
foreach(var source in sources) {
Assert.That(library.syz_handleDecRef(source), Is.EqualTo(0));
}
foreach(var reverb in reverbs) {
Assert.That(library.syz_handleDecRef(reverb), Is.EqualTo(0));
}
Assert.That(library.syz_shutdown(), Is.EqualTo(0));
Assert.Pass();
}
